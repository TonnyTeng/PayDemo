...
@property (nonatomic,retain) NSTimer *itTimer;
@property (nonatomic,retain) id<CommunicationCallBack> communication;
@property (nonatomic,retain) NSString *cardMsg;
@property (nonatomic,retain) NSString *pinMsg;
...
/*!
 @method
 @abstract 更新TMK02  0926
 @discussion 更新工作秘钥
 @param indexs 主秘钥对应的索引值
 @param Mainkeys 多组主秘钥
 @param time 超时时间
 
 */
-(void) Request_ReNewTMK2:(NSArray *)indexs MainKeys:(NSArray *)MainKeys time:(int)_timeOut;

/**
 同步时间0931
 */
- (void)Request_SetDateTime:(char *)data Lenght:(int)_len timeOut:(int)_timeOut;
/**
 获取设备时间0932
 */
- (void)Request_GetDateTime;
/**
 保存数据0933
 */
- (void)Request_SaveDataToDevice:(char *)data length:(int)len timeOut:(int)_timeOut;
/**
 获取数据0934
 */
- (void)Request_GetDataFromDevice:(int)index;
- (void)Request_GetDataFromDevice1:(NSArray *)arr;

//获取自增流水号0935
- (void)Request_FlowNumber;
//保存数据内容0936
- (void)saveData:(char *)data dataLen:(int)len timeOut:(int)time;
//PIN转加密0220
- (void)TransPinEnc:(char *)pinData pinLen:(int)pinlen ramdom:(char *)data ramLen:(int)rLen tiemOut:(int)_timeOut;
/*!
 @method
...
-(void) stat_EmvSwiper:(int)mode
           ...
           MACKeyIndex:(int)_MACKeyIndex
              CtrlMode:(char*)_CtrlMode
        TitleInPutCard:(char *)cardTitle
          titleCardLen:(int)tCardLen
         TitleInPutPwd:(char *)pwd
           titlePwdLen:(int)tPwdLen
       ParameterRandom:(char*)_ParameterRandom
    ParameterRandomLen:(int)_ParameterRandomLen
                  cash:(char*)_cash
...
/*
 ...
 bit1 = 0/1卡号加密时不压缩/压缩（当字节3的bit0=1时且字节2的bit3=1，或者当字节2的bit6=1时，该标志有作用）
 bit2 =0/1  不上送/上送卡有效期
 bit3 =0/1  PIN参与/不参与mac计算
 bit4= 0/1 PAN或卡有效期 (如果要求上送时)包含在磁道数据尾部/单独作为独立字段上送（PAN定长8字节，卡有效期为定长4字节，跟随在磁道密文后面）
 bit5 =0/1  MAC数据组包时的附加数据域在头部/在尾部
 bit6 =0/1标准MAC控制/银嘉定制
 Bit7 =0/1表示无自定义标题/有自定义标题
 字节4：
 Bit0：=0/1不上送/上送进入密码输入状态。
 Random 随机数
 Appenddata 附加数据域
 Time 超时时间
 Transactioninfo IC卡交易参数
